          ...
          void encoder_init(void){
            ...
            pinMode(interruptPIN2, INPUT);//INPUT_PULLUP);
                  pinMode(interruptPIN3, INPUT);//INPUT_PULLUP);
                  attachInterrupt(digitalPinToInterrupt(interruptPIN2), encoder0CHA, CHANGE);//set GPIO pin for encoder
                  attachInterrupt(digitalPinToInterrupt(21), encoder0CHB, CHANGE);//set GPIO pin for encoder


                  
                  pinMode(interruptPIN0,INPUT);//INPUT_PULLUP);
                  pinMode(interruptPIN1, INPUT);//INPUT_PULLUP);
                  attachInterrupt(digitalPinToInterrupt(interruptPIN0), encoder1CHA, CHANGE);//set  GPIO pin for encoder
                  attachInterrupt(digitalPinToInterrupt(20), encoder1CHB, CHANGE);//set  GPIO pin for encoder
          }
